...
export default {
  ...
  methods: {
    methodDialogBox(){
        console.log("entered in change of checkbox.")
      let check=true
      for(let i=0;i<Object.keys(this.form.checked).length;i++){
        console.log("Object.keys(this.form.checked)[i]:",Object.keys(this.form.checked)[i],"=>",this.form.checked[Object.keys(this.form.checked)[i]])
    ...
    createEssentialFiles(newFolderName) {
      ...
      let menu = newFolderName + '/Menu/default.menu'
      axios.post(config.baseURL + '/flows-dir-listing', {
          filename : menu,
          text : '',
          type : 'file'
        ...
        axios.delete(config.baseURL + '/flows-dir-listing/0?filename=' + data.path.replace(/\\/g, "/"))
          .then((res) => {
            ...
            if (_.includes(data.path, 'Pages')) {
              ...
              this.globalConfigData[2].layoutOptions[0].Pages.splice(indexOfPageName, 1);
              ...
              for (var i = 0; i < partialsArray.length; i++){
                var partialNameBreak = last_element.replace('.html','');
                var partialNameOnly = partialNameBreak;
                console.log("last_element:",last_element)
                var partialName=file_path_.split('\\')[file_path_.split('\\').length -2]
